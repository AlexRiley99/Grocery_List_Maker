document.addEventListener("DOMContentLoaded", function () {
  //Ensures that HTML loads before JS executes
  ...
  function SetBudgetButtonClick() {
    ...
    if (budgetValue.trim() == "") {
      ...
      //If value is not a number, display alert
      alert("Please enter a valid numeric value for your budget amount.");
    } else {
      budget.innerHTML =
        "<h4 class='calcValues'>$" +
        parseFloat(budgetValue).toFixed(2) +
        "</h4>";
  ...
  function budgetOverUnder() {
    //Displays a message if the user is over, under, or on budget
    ...
    if (totalValue < budgetValue) {
      overUnder.innerHTML =
        "<h4 class='overUnderOutput' id='underBudget'>You are under budget by $" +
        differenceValue.toFixed(2) +
        "! Way to save money!</h4>";
    } else if (totalValue > budgetValue) {
      overUnder.innerHTML =
        "<h4 class='overUnderOutput' id='overBudget'>Oops! You're over budget by $" +
        Math.abs(differenceValue).toFixed(2) +
        "! You may edit your list by clicking the Edit button.</h4>";
    } else if (differenceValue == 0) {
      overUnder.innerHTML =
        "<h4 class='overUnderOutput' id='onBudget'>You're right on budget!</h4>";
    }
  }
  ...
  function addToList() {
    ...
    if (item.trim() !== "" && quantity.trim() !== "" && cost.trim() !== "") {
      //check if input is not empty
      if (budgetAmount.value.trim() == "") {
        //Make sure the user has a budget amount set
  ...
  function AddButtonClick() {
    //Adds items to list and displays values after "Add is clicked"
    addToList();
    displayList();
  ...
  function calculateDifference() {
    ...
    var differenceAmount =
      parseFloat(budgetValue).toFixed(2) - parseFloat(finalTotal).toFixed(2);
  ...
  function displayTotal() {
    ...
    total.innerHTML =
      "<h4 class='calcValues'>$" + runningTotal.toFixed(2) + "</h4>"; //display total
  }
  ...
  function displayDifference() {
    ...
    difference.innerHTML =
      "<h4 class='calcValues'>$" + differenceAmount.toFixed(2) + "</h4>";
  }
  ...
  function EditButtonClick() {
    //Opens a text box that receives an item number that will then correspond to an index
    var listEditorDiv = document.getElementById("listEditorDiv");
    listEditorDiv.style.display = "block";
  }
  editButton.addEventListener("click", EditButtonClick);

  function ConfirmEditButtonClick() {
    //Confirm user input and display corresponding array information
    ...
    if (
      ...
      index >= quantArray.length + 1 ||
      index >= costArray.length
    ) {
      alert(
        'Please enter a valid item number. You can refer to "Your List" for the range of valid numbers'
      );
  ...
  function confirmDelete() {
    var confirmed = window.confirm(
      "Are you sure you want to delete this list? It cannot be recovered."
    );
    if (confirmed) {
      location.reload();
    }
